...
async def test_se_baixa_historico_de_uma_estacao_pluviometricos(crawler: Crawler, chuva_repository:RainRepository):
...
@pytest.mark.skipif(site_online, reason="Temporariamente desativado")
async def test_se_baixa_historico_pluviometricos_todas_estacoes(crawler: Crawler, chuva_repository:RainRepository):
    crawler = await crawler.download_rainfall_history(
